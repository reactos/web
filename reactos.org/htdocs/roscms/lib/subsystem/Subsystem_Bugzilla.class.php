...
class Subsystem_Bugzilla extends SubsystemExternal
{
  ...
  const DB_NAME = 'bugs'; // change this to your Bugzilla-DB name
  ...
  /**
   *  checks if user Details are matching in roscms with Bugzilla
   *
   * @access protected
   */
  ...
  /**
   * update user details in the Bugzilla database
   *
   * @param int user_id
  ...
  /**
   * add a new user to the bugzilla database
   * add mapping
   *
   * @param int user_id
   * @param string name
  ...
  /**
   * add a new mapping for this user to bugzilla
   *
   * @param int user_id
   * @return bool
   * @access protected
  ...
  protected function addMapping( $user_id )
  {
    ...
    // Second, try to find user by name
    if ($bz_user_id === false) {
    ...
    // update bugzilla data / insert mapping
    else {
